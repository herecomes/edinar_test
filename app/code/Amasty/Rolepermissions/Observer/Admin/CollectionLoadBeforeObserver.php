...
class CollectionLoadBeforeObserver implements ObserverInterface
{
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        ...
        if ($rule->getScopeStoreviews()) {
            ...
            //restriction by stores on marketing -> newsletter subscribers
            if ($collection instanceof \Magento\Newsletter\Model\ResourceModel\Subscriber\Collection) {
                $allowedStoreViews = $rule->getScopeStoreviews();
                $collection->addFieldToFilter('store.store_id', ['in' => $allowedStoreViews]);
            }
            ...
            if ($collection instanceof \Magento\Sales\Model\ResourceModel\Order\Payment\Transaction\Collection) {
                ...
                $collection->getSelect()->join(
                    ['am_review_store' => $collection->getTable('review_store')],
                    'am_review_store.review_id = rt.review_id',
                    []
                )->where('am_review_store.store_id IN (?)', $ids);
